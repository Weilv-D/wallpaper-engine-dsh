...
一个 DSH bundle：把本机 **Wallpaper Engine** 壁纸库变成 DSH web GUI(`dsh web`)的动态背景。视频壁纸在聊天界面背后播放，网页壁纸原地渲染——带交叉淡入、液态玻璃面板、轮播列表、搜索和资源监控。

## 功能

- **只列可原生渲染的壁纸** —— 视频与网页壁纸动态渲染,且是列表中仅有的两类:场景/应用壁纸是 Wallpaper Engine 私有的 `.pkg` 场景包,浏览器无法渲染,因此直接从列表中过滤,而不是以降级的静态图充数。浏览器解不了的视频会临时退到预览图,下次刷新即重试原媒体。
...
- **任意壁纸都可读** —— 遮罩颜色与文字颜色相对（浅色主题提亮、深色主题压暗）;"智能可视"开关每隔几秒采样一次壁纸亮度，画面过亮或过暗时自动加强遮罩。下拉菜单、弹窗等浮层保持 DSH 原生不透明表面，不受影响。
- **手动裁剪** —— 先选基础排布（填充/适应/拉伸/原始），再自己取景：点「调整画面」把壁纸全屏展开，拖动平移、滚轮缩放，「重置裁剪」一键还原。
...
3. 用滑杆调节融合效果。壁纸太花时调高 **暗化** 和 **边框** 直到文字清晰；界面跟随 DSH 明暗主题自动适配。**排布** 一行决定视频与静态壁纸的基础摆放（填充/适应/拉伸/原始），点「调整画面」可在画布上直接拖动平移、滚轮缩放取景。
...
- **浏览器插件** 把选中的壁纸渲染在应用框架之下的固定层，提供设置界面，所有视觉效果都读 DSH 设计令牌，主题切换自然生效。壁纸模糊采用裁边式超采样（内容不变形），叠加微量抖动噪声消除渐变色带；反复解码失败的视频（不支持的容器）退到预览图，下次刷新重试原媒体。

清单由宿主缓存 30 秒，媒体令牌按壁纸稳定复用（后台重建不会掐断播放中的流）；安装发现异步执行并缓存 10 分钟。本插件不注册模型工具，不占用 prompt。

## 限制

- 场景与应用壁纸完全不出现在列表里：它们是 Wallpaper Engine 私有的 `.pkg` 场景包，动态渲染只能由桌面端的 Wallpaper Engine 完成。
