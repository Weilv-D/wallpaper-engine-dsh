 ...
 *   statSync (default to node:fs), plus:
 *   - `mint(entry)` — mint a media/preview token for a file entry; the host
 *     provides a random-token map, tests provide a spy.
 *   - `base` — URL prefix for media/preview links (default '/we-background').
 * @returns {{ installDir, total, portableCount, wallpapers, playlists }}
 */
export function buildInventoryFrom(discovery = {}, io = {}) {
  const exists = io.existsSync || existsSync;
  const read = io.readFileSync || readFileSync;
  ...
  const roots = wallpaperRoots(installDir, libraryRoots, { existsSync: exists });
  const projects = enumerateWallpapers(roots, io);
  ...
  const wallpapers = projects.map((p) => {
    ...
    const hasMedia = portable && exists(p.fileAbs);
    const hasPreview = Boolean(p.previewAbs) && exists(p.previewAbs);
    return {
      id: p.id,
      title: p.title,
      type: p.type,
      playable: hasMedia,
      media: hasMedia
        ? `${base}/media/${mint({ abs: p.fileAbs, rootDir: p.type === 'web' ? p.dir : null })}`
        : null,
      preview: hasPreview
  ...
  if (configPath && exists(configPath)) {
    try {
      playlists = parsePlaylists(JSON.parse(read(configPath, 'utf8')));
