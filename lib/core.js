...
import { join, resolve, normalize, relative, basename, sep } from 'node:path';
...
/**
 * A filesystem-relative path (`bin\index.html`, `我的 壁纸.html`) as URL path
 * segments (`bin/index.html`, `%E6%88%91…`). Each segment is encoded
 * separately so `/` stays a separator and spaces/unicode survive the round
 * trip through the route's decodeURIComponent.
 * @param {string} fsPath
 * @returns {string}
 */
export function toUrlPath(fsPath) {
  return String(fsPath).split(/[\\/]/).filter(Boolean).map(encodeURIComponent).join('/');
}
...
export async function buildInventoryFrom(discovery = {}, io = {}) {
  ...
  for (const p of projects) {
    ...
    const previewToken = hasPreview
      ...
      : null;
    // Web wallpapers are addressed AT their entry file (`…/media/<token>/index.html`):
    // the document URL then mirrors the on-disk layout of the project, so the
    // wallpaper's own relative references (scripts, styles, images, fetches)
    // resolve exactly where Wallpaper Engine itself would resolve them. A bare
    // token URL would strand `app.js` at `…/media/app.js` — one directory too
    // high, a 404, and a blank wallpaper. Videos keep the bare form: browsers
    // never resolve relative URLs against a <video> src.
    const mediaSuffix = p.type === 'web' && mediaToken
      ? '/' + toUrlPath(relative(p.dir, p.fileAbs))
      : '';
    wallpapers.push({
      id: p.id,
      title: p.title,
      ...
      media: hasMedia && mediaToken ? `${base}/media/${mediaToken}${mediaSuffix}` : null,
  ...
  const playlistView = playlists.map((playlist, index) => {
    ...
    return {
      ...
      wallpaperIds: ids,
      // ids only ever contains playable entries (the filter above), so
      // every resolved item is portable by construction.
      total: ids.length,
      portableCount: ids.length,
