...
// ── 6. Image dimension sniffing ─────────────────────────────────────────────

/**
 * Read pixel dimensions from an image's header bytes (JPEG/PNG/GIF/WebP).
 * Used to tell the client how sharp a scene/application preview really is —
 * workshop previews range from 256px thumbnails to full renders, and the UI
 * presents low-resolution ones differently.
 *
 * Header-only: reads at most 64 KiB, never decodes pixels. Returns
 * `{ w, h }` or null for unrecognized/truncated data.
 *
 * @param {Buffer|Uint8Array} b
 * @returns {{ w: number, h: number } | null}
 */
export function sniffImageSize(b) {
  if (!b || b.length < 10) return null; // GIF needs 10; other formats check their own reach
  // PNG: 8-byte signature, then IHDR with width/height big-endian at 16/20.
  if (b[0] === 0x89 && b[1] === 0x50 && b[2] === 0x4E && b[3] === 0x47) {
    if (b.length < 24) return null;
    return { w: b.readUInt32BE(16), h: b.readUInt32BE(20) };
  }
  // GIF: logical screen dimensions little-endian at 6/8.
  const sig6 = b.toString('latin1', 0, 6);
  if (sig6 === 'GIF87a' || sig6 === 'GIF89a') {
    return { w: b.readUInt16LE(6), h: b.readUInt16LE(8) };
  }
  // JPEG: walk markers to the first start-of-frame segment.
  ...
  // WebP: RIFF container; VP8X carries canvas size minus one, uint24 LE.
  if (b.toString('latin1', 0, 4) === 'RIFF' && b.toString('latin1', 8, 12) === 'WEBP' &&
      b.toString('latin1', 12, 16) === 'VP8X' && b.length >= 30) {
    return {
...
/** Kinds whose only browser-renderable asset is the preview image. */
const PREVIEW_ONLY_KINDS = new Set(['scene', 'application']);
...
export async function buildInventoryFrom(discovery = {}, io = {}) {
  ...
  for (const p of projects) {
    const portable = PORTABLE_KINDS.includes(p.type);
    const hasMedia = portable && await exists(p.fileAbs);
    ...
    const previewToken = hasPreview
      ...
      : null;
    // Preview pixel size matters for scene/application wallpapers: the
    // preview is ALL the browser can show, so the client needs to know how
    // sharp it is. Sniff only for preview-only kinds (videos don't care).
    let previewSize = null;
    if (hasPreview && PREVIEW_ONLY_KINDS.has(p.type) &&
        typeof io.readBuffer === 'function') {
      try { previewSize = sniffImageSize(await io.readBuffer(p.previewAbs, 65536)); }
      catch { previewSize = null; }
    }
    wallpapers.push({
      id: p.id,
      title: p.title,
      ...
      previewW: previewSize ? previewSize.w : null,
      previewH: previewSize ? previewSize.h : null,
    });
  }
