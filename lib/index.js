...
const asyncIo = {
  ...
  // Header-only binary read for the image dimension sniffer (64 KiB cap).
  readBuffer: async (p, max) => {
    const fh = await fsp.open(p, 'r');
    try {
      const buf = Buffer.alloc(Math.min(max, 65536));
...
export function createRouteRegistrar(webServer, opts = {}) {
  ...
  function getInventory(force) {
    ...
    // Coalesce concurrent builds (double-clicked 刷新, parallel clients).
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      getInventory(url.searchParams.get('refresh') === '1')
        ...
        .catch((err) => {
          sendJson(req, res, 500, { error: String(err && err.message ? err.message : err) });
        });
    },
  }));
