  ...
  function serveFile(absPath, req, res) {
    fsp.stat(absPath).then((st) => {
      ...
      if (mime === 'text/html' || mime === 'image/svg+xml') {
        ...
        // sandboxed CSP even on direct top-level navigation, so they can
        // never execute against the DSH origin. The client's iframe sandbox
        // already covers embedding; this is the server-side backstop.
