...
function readPersisted() {
  try {
    ...
    return {
      ...
      zoom: Math.max(1, clampNum(o.zoom, 0.25, 4, DEFAULTS.zoom)),
      // Pan may have been persisted with a larger headroom than the zoom
      // allows (older builds let zoom go below 1) — clampCrop re-seats it.
...
function attachStillSampling(media, sel) {
  const type = (sel && sel.type) || (media.tagName === "IMG" ? "image" : null);
...
function WallpaperPicker() {
  ...
  return React.createElement("div", { className: "webg-picker" },
    ...
    sel.id && React.createElement(React.Fragment, null,
      ...
      // ignores object-fit and always fills its layer. Manual crop (drag /
      // wheel in the adjust overlay) replaced the old fixed anchor select.
