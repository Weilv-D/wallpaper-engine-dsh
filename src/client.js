 ...
 *
 * What it does:
 *   1. Fetches the wallpaper inventory from the host half (same-origin
 *      GET /we-background/inventory). 刷新 forces a host-side rebuild
 *      (?refresh=1) so newly subscribed Workshop items appear without a
 *      page reload.
 *   2. Renders the selected wallpaper BEHIND the DSH GUI — a fixed layer
 *      under the app frame, with a crossfade on switch and a scrim for
 *      legibility. Web wallpapers render in a SANDBOXED iframe
 *      (`allow-scripts`, no allow-same-origin): third-party wallpaper JS
 *      gets an opaque origin and cannot touch DSH storage or APIs.
 *   3. Four live sliders (壁纸模糊 / 暗化 / 边框 / 玻璃) tune the blend with
 *      the UI; all read DSH design tokens so light/dark theme switches just
 *      work.
 *   4. Automatic rotation over user-defined carousel lists (轮播列表), each
 *      with its own interval and order, persisted in localStorage. A playable
 *      Wallpaper Engine playlist is imported as the first list on first run.
 */
